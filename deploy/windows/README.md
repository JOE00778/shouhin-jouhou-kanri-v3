...
│ 进程                    │ RAM 上限  │ CPU 上限 │
├─────────────────────────┼───────────┼──────────┤
│ Postgres                │  512 MB   │  0.5 核  │
│ Streamlit               │ 1024 MB   │  1.0 核  │
│ Cloudflared             │  128 MB   │ 0.25 核  │
│ Docker Desktop 自身      │  300 MB   │  0.25 核 │
├─────────────────────────┼───────────┼──────────┤
│ CMS 总占用              │ ~2 GB     │  ~2 核   │
├─────────────────────────┼───────────┼──────────┤
│ 影刀 + Windows 系统     │ 6 GB+     │  2 核+   │  ← 8GB / 4核 笔记本剩余
└─────────────────────────┴───────────┴──────────┘
```

通过 `.wslconfig` 全局限制 WSL2，CMS 永远抢不到比这更多。**影刀进程优先级永远高于 Docker。**
