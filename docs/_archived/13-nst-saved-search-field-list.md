...
**计算规则（Boss 2026-05-10 规则）：**

```
弁天在庫金額         = 平均原価 × 弁天.手持             # 弁天 没有在途
JD在庫金額           = 平均原価 × JD.手持
全手持在庫金額       = 弁天在庫金額 + JD在庫金額

弁天在途金額         = 0                               # 弁天 没有 / 不会有在途
JD在途金額           = 平均原価 × (JD.注文待ち + JD.輸送中)
全在途在庫金額       = JD在途金額  (+ 其他仓库在途, 如有)

全在庫金額合計       = 全手持在庫金額 + 全在途在庫金額   ← `item_v2.total_amount`
```

**关键规则**：
- 弁天倉庫 是退品暂存仓, 物流上不会有发往弁天的在途订单, 所以 `注文待ち / 輸送中 = 0`
- HENPIN-EX bin 是已收货的退品实物, **不算在途**, 算手持
- JD-物流-千葉 / 其他仓库 才有在途库存 (注文待ち + 輸送中)
