...
c1.metric("🔥 重点提价", int(counts.get("🔥 重点提价", 0)))
c2.metric("🔥 重点降价", int(counts.get("🔥 重点降价", 0)))
c3.metric("⬆️ 提价候选", int(counts.get("⬆️ 提价候选", 0)))
c4.metric("⚠️ 降价候选", int(counts.get("⚠️ 降价候选", 0)))
c5.metric("⬇️ 降级候选", int(counts.get("⬇️ 降级候选", 0)))
c6.metric("✅ 维持", int(counts.get("✅ 维持", 0)))

st.divider()

# 等级 × 建议矩阵
st.markdown("### 等级 × 建议 矩阵")
matrix = pd.crosstab(df["rank"], df["advice"], margins=True, margins_name="合计")
...
def _show(filtered_df: pd.DataFrame, top_n: int = 100):
    ...
    st.caption(f"显示前 {min(len(show), top_n)} 行 / 共 {len(filtered_df)} 条")


with tabs[0]:
    st.markdown("**周转低 × 毛利高 — 降价加速周转 / 库存价值降序**")
    _show(df[df["advice"] == "🔥 重点降价"])

with tabs[1]:
    st.markdown("**周转高 × 毛利低 — 提价不影响销量 / 即时增毛利**")
    _show(df[df["advice"] == "🔥 重点提价"])

with tabs[2]:
    st.markdown("**周转低 × 毛利低 — 双低 · 等级下调候选（B→C / C→停售）**")
    st.caption("注：与改廃情報（page 13）不同 · 改廃 = 品牌方迭代外部信号 · 此处为内部数据驱动的渐变降级")
    _show(df[df["advice"] == "⬇️ 降级候选"])

with tabs[3]:
    st.markdown("**全部 1,681 条建议**")
