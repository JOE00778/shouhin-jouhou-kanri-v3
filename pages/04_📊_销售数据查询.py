...
# ============================================================
# 库存数据源优先级 (按 page 22 实测真实有数据的表):
#   1) item_inventory_snapshot_v2 (jan 字段名, 多仓 + bin)
#   2) inventory_snapshot (upc 字段名, 旧版单仓快照)
#   3) nst_inventory_snapshot (upc 字段名, NetSuite 直接导出, 通常 0 行)
# location 用 LIKE 'JD%' 兼容多种命名变体; % 字面量必须用 named param 传
def _try_query(sql, params=None):
    try:
        return conn.execute(sql, params or {}).fetchall(), None
...
def _query_inv():
    sources = [
        ("item_inventory_snapshot_v2",
         "SELECT item_code, jan AS upc, qty_on_hand, total_amount, "
         "location, department FROM item_inventory_snapshot_v2 "
         "WHERE location LIKE :loc_pattern"),
        ("inventory_snapshot",
         "SELECT item_code, upc, qty_on_hand, total_amount, "
         "location, department FROM inventory_snapshot "
         "WHERE location LIKE :loc_pattern"),
        ("nst_inventory_snapshot",
         "SELECT item_code, upc, qty_on_hand, total_amount, "
         "location, department FROM nst_inventory_snapshot "
         "WHERE location LIKE :loc_pattern"),
    ]
    for name, sql in sources:
        rs, _ = _try_query(sql, {"loc_pattern": "JD%"})
        if rs:
            return pd.DataFrame([dict(r) for r in rs]), name
    return pd.DataFrame(), "(all_empty)"

df_inv, _inv_src = _query_inv()
# 详细诊断 expander: 显示三个上游表的实际状态
with st.expander(t("🔬 数据源状态诊断 (上传后展开看是否真入库)"), expanded=False):
    _inv_rows, _ = _try_query("SELECT COUNT(*) AS c FROM nst_inventory_snapshot")
    _inv_total = _inv_rows[0]["c"] if _inv_rows else 0
    _turn_rows, _ = _try_query("SELECT COUNT(*) AS c FROM nst_turnover")
    _turn_total = _turn_rows[0]["c"] if _turn_rows else 0
    _sales_rows, _ = _try_query("SELECT COUNT(*) AS c FROM sales_line")
    _sales_total = _sales_rows[0]["c"] if _sales_rows else 0
    st.caption(t(f"📍 库存数据实际从 `{_inv_src}` 取得 ({len(df_inv):,} 行 JD%)"))
...
if df_inv.empty:
    st.warning(t(
        "⚠️ 三个库存表 (item_inventory_snapshot_v2 / inventory_snapshot / "
        "nst_inventory_snapshot) 都没有 location LIKE 'JD%' 的行。"
        "请到「🗄️ 数据库管理」看哪张表有数据 + 实际 location 命名。"
    ))
if _turn_total == 0:
    st.warning(t(
        "⚠️ `nst_turnover` 表为空,库存周转率/平均在庫日数/交叉比率列将全部 0。"
        "请上传 NetSuite 在庫回転率导出 (在庫回転率-xxx.xls)。"
    ))
if not df_inv.empty:
